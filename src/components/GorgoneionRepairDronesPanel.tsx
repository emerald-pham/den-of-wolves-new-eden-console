...
export default function GorgoneionRepairDronesPanel() {
  ...
  const usedThisCycle = repairHistoryValid && ledger!.cycle === currentCycle;
  ...
  useEffect(() => {
    ...
    setError(null);
    setStatus(null);
  }, [hostShipId, currentCycle, repairRevision, identity]);
    ...
    <section className="role-brief__rules gorgoneion-repair-drones" aria-labelledby="gorg-repair-title">
      ...
      <div className="maintenance-controls__confirmation">
        <button className="cic-action-button" type="button"
          ...
          onClick={() => void submit()}>
          {pendingForCurrentAuthority ? 'Repairing console…' : retryCommand ? 'Retry exact repair request' : 'Repair one console'}
        </button>
      </div>
      {errorMessage && <p role="alert">{errorMessage}</p>}
      {retryCommand && !pendingForCurrentAuthority && <p role="status">Retry the last request with its original request id.</p>}
