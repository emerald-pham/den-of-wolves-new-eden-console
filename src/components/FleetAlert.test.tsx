...
it('runs the Admiral command, waits for authority, then offers stand down', async () => {
  render(<><AegisConsoleWorkspace roleId="admiral" galacticCoordinate="0000" fuel={0} /><FleetBroadcast /></>);
  fireEvent.click(screen.getByRole('button', { name: 'Fleetwide red alert' }));
  ...
  expect(screen.getByText(/wolf attack imminent/)).toBeVisible();
...
it('does not offer the command to other roles and shows fleet messages to them', () => {
  ...
  expect(screen.getByText(/red alert cancelled by AEGIS/)).toBeVisible();
});
it('disables offline commands and reports server failures', async () => {
  ...
  render(<AegisConsoleWorkspace roleId="admiral" galacticCoordinate="0000" fuel={0} />);
  fireEvent.click(screen.getByRole('button', { name: 'Fleetwide red alert' }));
  ...
  expect(screen.getByRole('button', { name: 'Fleetwide red alert' })).toBeDisabled();
});
