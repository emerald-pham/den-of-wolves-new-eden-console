...
export default function FleetSystemsWorkspace({ ship, role, fuel, galacticCoordinate }: {
  readonly ship: Ship; readonly role: ConsoleRole; readonly fuel: number; readonly galacticCoordinate: string;
}) {
  ...
  return <RoleConsoleTemplate label={`${ship.name} ${role.name} console`}
    ...
    <p>Charges, damage, upgrades and procedure outcomes are tracked at the table.</p>
    {page === 'systems' && <div className="aegis-system-grid">{systems.map(system => <article className="aegis-system cic-frame" key={system.card}>
