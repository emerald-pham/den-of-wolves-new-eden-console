...
export default function ChacauRepairPanel({ control, docking, fuelled }: Props) {
  ...
  const isHolder = control.shuttleId === 'chacau' &&
    control.ownerRoleId === 'refinery-124-engineer' && me.role === 'player' &&
    me.uid === control.holderUid && me.assignedRoleId === 'refinery-124-engineer';
  ...
  async function submitRepair(): Promise<void> {
    ...
    const command: ChacauRepairCommand = activeRetry?.command ?? {
    ...
    try {
      const result = await repairConsolesFromChacau(command);
