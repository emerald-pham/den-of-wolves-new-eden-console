...
describe('parseMaliadesState', () => {
  it('returns an immutable unlaunched state when the legacy field is absent', () => {
    ...
    expect(state).toEqual({ revision: 0, launched: false, damage: 0, destroyed: false, medium: null, short: null });
  });

  it('accepts a canonical launch and resolution projection', () => {
    expect(parseMaliadesState({
      revision: 2, launched: true, damage: 1, destroyed: false,
  ...
  it('fails closed for forged or impossible snapshots', () => {
    expect(parseMaliadesState({ revision: 1, launched: false, damage: 0, destroyed: false, medium: null, short: null })).toBeUndefined();
    expect(parseMaliadesState({
      revision: 2, launched: true, damage: 1, destroyed: false,
      medium: null, short: null,
    })).toBeUndefined();
    expect(parseMaliadesState({
      revision: 2, launched: true, damage: 0, destroyed: false,
