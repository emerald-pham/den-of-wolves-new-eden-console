...
export const PHILIA_REPAIR_COST = 4;
export const PHILIA_MAX_CONSOLES_PER_SHIP = 2;

export interface PhiliaRepairHostEntry {
...
export interface PhiliaRepairLedger {
  ...
  readonly hosts: readonly PhiliaRepairHostEntry[];
}
...
export function parsePhiliaRepairLedger(value: unknown): PhiliaRepairLedger | null {
  ...
  const hosts: PhiliaRepairHostEntry[] = [];
  for (const host of raw.hosts) {
    ...
    if (!entry || Object.keys(entry).some((key) => !['shipId', 'systemIds'].includes(key)) ||
        ...
        entry.systemIds.length > PHILIA_MAX_CONSOLES_PER_SHIP ||
...
export function resolvePhiliaRepair(input: Readonly<{
  actorUid: string;
  currentCycle: number;
  ...
  if (input.control.shuttleId !== 'philia' ||
      input.control.ownerRoleId !== 'dione-engineer' ||
      input.control.holderUid !== input.actorUid) {
    throw new Error('Only the current Philia holder may repair consoles.');
  }
  if (input.control.revision !== input.expectedControlRevision) {
    throw new Error('Philia control changed; refresh before repairing.');
  }
  if (!Number.isSafeInteger(input.currentCycle) || input.currentCycle < 1) {
    throw new Error('A numbered cycle is required for Philia repairs.');
  }
  if (input.ledger.revision !== input.expectedRepairRevision) {
    throw new Error('Philia repair history changed; refresh before repairing.');
  }
  if (input.ledger.revision >= Number.MAX_SAFE_INTEGER || input.ledger.cycle > input.currentCycle) {
    throw new Error('Philia repair history is outside the safe cycle range.');
  }
  if (input.systemIds.length < 1 || input.systemIds.length > PHILIA_MAX_CONSOLES_PER_SHIP ||
  ...
  const currentDockings = input.dockings.filter((docking) => docking.shuttleId === 'philia');
  ...
  if (input.damage.destroyed) throw new Error('A destroyed ship cannot receive Philia repairs.');
  ...
  if (repairedOnHost.length + input.systemIds.length > PHILIA_MAX_CONSOLES_PER_SHIP) {
    throw new Error('Philia may repair at most two consoles on one ship each cycle.');
  }
  if (!priorHost && priorHosts.length >= 1 && !input.fuelled) {
    throw new Error('Fuel Philia before repairing a second ship this cycle.');
  }
  if (!priorHost && priorHosts.length >= 2) {
    throw new Error('Philia may repair at most two ships each cycle.');
  }
  const cost = input.systemIds.length * PHILIA_REPAIR_COST;
