...
const transit = (fields: Partial<ShuttleTransitState> = {}): ShuttleTransitState => ({
  ...
  routeLegs: [{ fromShipId: 'aegis', toShipId: 'dione',
    originPosition: { x: 0, y: 0, z: 0 }, destinationPosition: { x: -0.32, y: 0.18, z: 0.22 },
  ...
  const retargetedTransit = transit({
    ...
    routeLegs: [
      { fromShipId: 'aegis', toShipId: 'dione', originPosition: { x: 0, y: 0, z: 0 },
        destinationPosition: { x: -0.32, y: 0.18, z: 0.22 },
        departedAt, arrivesAt },
      { fromShipId: 'dione', toShipId: 'icebreaker',
        originPosition: { x: -0.16, y: 0.09, z: 0.11 },
        destinationPosition: { x: 0.26, y: -0.12, z: 0.28 },
        departedAt: '2026-09-21T18:00:30.000Z', arrivesAt: '2026-09-21T18:01:30.000Z' },
    ],
  });
  const result = resolveWolfAttackShuttleParking({
...
it('never crosses a split-fleet boundary even when another group ship is physically closer', () => {
  const splitTransit = transit({
    ...
    routeLegs: [{ fromShipId: 'quellon', toShipId: 'refinery-124',
      originPosition: { x: -0.28, y: -0.08, z: -0.26 },
      destinationPosition: { x: 0.09, y: 0.32, z: 0.31 }, departedAt, arrivesAt }],
    originPosition: { x: -0.28, y: -0.08, z: -0.26 },
