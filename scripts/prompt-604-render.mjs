...
const OUTPUT_DIR = resolve(process.env.PROMPT_604_EVIDENCE_DIR ?? '/tmp/prompt-604-render');
const VIEWPORTS = [
...
function persistedState(now) {
  const session = {
    id: 'prompt-603-narrow-console', name: 'Short Landscape Review', joinCode: '6040',
    ...
    fleetTicker: {
      revision: 1, nextSequence: 2, replayCursor: 1,
      current: {
        id: 'prompt-604:fleet-ticker:1', sequence: 1, source: 'automatic',
        priority: 40, sourceId: 'airspace:3:restricted',
...
async function inspect(page, viewport, motion) {
  ...
  await page.getByRole('status', { name: 'AIRSPACE CONTROL // AIRSPACE CLOSED' }).waitFor({ state: 'visible' });
  ...
  const targets = await page.evaluate(() => {
    ...
    return {
      ...
      ticker: (() => {
        const element = document.querySelector('.fleet-broadcast');
        const rect = element?.getBoundingClientRect();
        return rect ? {
  ...
  if ((responsiveViewport && targets.ticker.position !== 'fixed') ||
      !targets.ticker.text.includes('AIRSPACE CONTROL // AIRSPACE CLOSED') ||
      targets.ticker.left < -1 || targets.ticker.right > viewport.width + 1) {
    throw new Error(`${viewport.name}/${motion}: the live ticker is not pinned and readable in short-landscape review: ${JSON.stringify(targets.ticker)}`);
  }
  ...
  const maintenanceSteps = page.locator('.maintenance-systems__cycle > ol > li');
  const maintenanceStepCount = await maintenanceSteps.count();
  if (maintenanceStepCount !== 7) {
    throw new Error(`${viewport.name}/${motion}: expected all seven maintenance steps, got ${maintenanceStepCount}`);
  }
  for (let index = 0; index < maintenanceStepCount; index += 1) {
    const step = maintenanceSteps.nth(index);
    const stepTargets = step.locator(':is(.maintenance-systems__step, button, fieldset, [role="status"])');
    const stepTargetCount = await stepTargets.count();
    if (stepTargetCount === 0) {
      throw new Error(`${viewport.name}/${motion}: maintenance step ${index + 1} has no reachable label or result.`);
    }
    for (let targetIndex = 0; targetIndex < stepTargetCount; targetIndex += 1) {
      const target = stepTargets.nth(targetIndex);
      await target.scrollIntoViewIfNeeded();
      const targetState = await target.evaluate((element) => {
        const rect = element.getBoundingClientRect();
        const ticker = document.querySelector('.fleet-broadcast');
        const tickerRect = ticker?.getBoundingClientRect();
        return {
          text: (element.innerText ?? element.textContent ?? '').replace(/\s+/g, ' ').trim(),
          top: rect.top, bottom: rect.bottom, left: rect.left, right: rect.right,
          tickerOverlap: Boolean(tickerRect && rect.left < tickerRect.right && rect.right > tickerRect.left &&
            rect.top < tickerRect.bottom && rect.bottom > tickerRect.top),
          clipped: element.scrollWidth > element.clientWidth,
        };
      });
      if (!targetState.text || targetState.left < -1 || targetState.right > viewport.width + 1 ||
          targetState.tickerOverlap || targetState.clipped) {
        throw new Error(`${viewport.name}/${motion}: maintenance step ${index + 1} target ${targetIndex + 1} is obscured or clipped: ${JSON.stringify(targetState)}`);
      }
    }
  }
  ...
  const primaryActionState = await primaryAction.evaluate((element) => {
    const rect = element.getBoundingClientRect();
    const ticker = document.querySelector('.fleet-broadcast');
    const tickerRect = ticker?.getBoundingClientRect();
    return {
  ...
  if (primaryActionState.width < 44 || primaryActionState.height < 44 ||
      primaryActionState.left < -1 || primaryActionState.right > viewport.width + 1 ||
      primaryActionState.tabIndex < 0 || primaryActionState.tickerOverlap ||
      !/Oxanium|Share Tech Mono|monospace/i.test(primaryActionState.fontFamily)) {
    throw new Error(`${viewport.name}/${motion}: primary fleet action failed short-landscape readability checks: ${JSON.stringify(primaryActionState)}`);
  }
  ...
  try {
    ...
    for (const viewport of VIEWPORTS) {
      for (const motion of MOTION_MODES) {
        ...
        console.log(`Prompt 604 rendered proof passed: ${viewport.name} ${viewport.width}x${viewport.height} ${motion} motion`);
        await context.close();
      }
    }
