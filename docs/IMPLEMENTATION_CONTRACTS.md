...
| `AMB-11` | Same plan section, the authorized facilitation-source review, and Prompts 560-561 | Pursuit failure and total destruction of the canonical active full-ship roster are settled terminal failure triggers. Candidate N/O/P results, final survivor aggregation, other lost-ship states, and closure timing remain incomplete. | Permit the two settled failure transitions to freeze active play while preserving all survivor, pod, shuttle, small-craft, and audit ledgers. Keep candidate results, survivor totals, other loss sources, debrief completion, and `closed` transitions blocked until their dependent prompts record those decisions. |
...
| 089 | done | `src/lib/firestore.ts` replaces each server event snapshot on reconnect, derives stable IDs from Firestore document IDs, and `src/lib/firestore.test.ts` proves overlapping snapshots do not accumulate or accept payload ID spoofing; `src/routes/GmConsole.tsx` renders by that stable ID without replay side effects. |
...
| 091 | verified | `functions/src/turnZero.ts` and `src/types/game.ts` define the complete server-owned `turnState` projection with current/max turn, Team or Coordination phase, phase revision, and start/end instants. `functions/src/index.ts` keeps it aligned through start/advance, the Team-to-Coordination handoff, timer controls, and the legacy boundary; focused turn-zero, start, production matrix, transition, and client parser tests cover the schema and lifecycle. |
