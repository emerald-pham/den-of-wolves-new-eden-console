...
| 051 | partial | Client/server catalog and helper tests now prove the settled 8–20 rows, while production-path creation, hydration, casting, readiness, start, and reconnect composition remains open. Earlier green 19/21 counted-Press rows are contract drift and cannot satisfy the owner-set 19/20 Capybara core contract. Target exact high rows, optional non-counting Press, and one/optional-multiple GMs. |
