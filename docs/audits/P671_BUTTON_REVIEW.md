...
Additional settings markup was captured after opening the real AppHeader dialog in its interaction fixture, then rendered with production-order CSS at the same three sizes and both motion preferences. All visible dialog buttons and all visible shared buttons on the GM and ship fixtures meet both 44px dimensions. Census/rule-call save controls and the opened settings dialog were visually inspected. Reconciled onto the verified 0.3.96 release for the 0.3.97 release candidate; final test/build and deployment results are tracked separately.
