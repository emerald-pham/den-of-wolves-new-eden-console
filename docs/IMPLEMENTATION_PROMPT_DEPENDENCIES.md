...
| 140d | NEW | partial | 131 | none | none | none | none | none | none | none | E-AUDIT-140D;E-140D-VERIFIED | M3;M11 | Create escape pods on ship destruction. |
...
| 145 | EXTEND | partial | 141 | none | none | none | none | none | none | none | E-AUDIT-145 | M2 | Lock airspace for a Wolf attack. |
| 146 | DECISION | missing | none | none | none | none | none | none | none | none | none | M2 | Decide nearest-ship parking ties. |
| 147 | EXTEND | partial | none | none | none | none | none | none | none | 432;433 | E-147-RELATED | M2 | Restrict battle-table craft. |
...
| 164 | PRESERVE | partial | 161;162 | none | none | none | none | none | none | none | E-AUDIT-164;E-164-ALLOWLIST-PARTIAL | M3;M5 | Encode cargo permissions. |
...
| 272 | PRESERVE | partial | 161;162 | none | none | none | none | none | none | none | E-AUDIT-272 | M3;M5;M6 | Register Chepu completely. |
...
| 305 | NEW | missing | 103;485 | none | none | none | none | none | none | none | E-AUDIT-305;E-305-GROUP-FOUNDATION | M4;M5 | Advance pursuit each turn. |
...
| 314 | NEW | partial | 285;140d | none | none | none | none | none | none | none | E-AUDIT-314;E-314-POD-PRODUCER;E-314-NAVIGATION | M4;M5 | Remove destroyed ships from navigation. |
...
| 416a | NEW | partial | none | none | none | none | none | none | none | none | E-416A-SOURCE-PARTIAL | M7 | Encode Ice Asteroids B. |
| 416b | NEW | partial | none | none | none | none | none | none | none | none | E-416B-SOURCE-PARTIAL | M7 | Encode Rare Element Moon C. |
...
| 432 | NEW | partial | 427;428 | none | none | none | none | WOLF-ATTACK | none | 146;373;429;430 | E-WOLF;E-AUDIT-432;E-432-DECLARATION | M9 | Declare the attack atomically. |
...
| 435 | NEW | partial | 432;515 | none | none | none | none | WOLF-ATTACK | none | none | E-WOLF;E-432-DECLARATION;E-515-REPLACEMENT;E-AUDIT-435 | M9 | Resolve Wolf Commander target rerolls. |
...
| E-AUDIT-145 | hard_prompt | 145 -> 141 | functions/src/index.ts; functions/src/wolfAttackDeclaration.ts; functions/src/wolfAttackDeclarationCallable.test.ts; functions/src/jumpCallable.test.ts; functions/src/actionMetadata.ts; functions/src/actionMetadata.test.ts; firestore.rules; tests/rules/firestore.rules.test.ts; authorized routed REFERENCE_ONLY_WOLF_ATTACKS.md - The sequence | P432 now fails closed unless every represented shuttle is validly docked and every fighter-wing bay record is present, then atomically restricts airspace and snapshots all parked craft; ordinary movement and jump callables reject the restricted phase. Relocation to nearest hosts and equal-distance ties remain unresolved under P146/P373. |
...
| E-165-METADATA-PARTIAL | evidence | 165 -> 161;162;130;122;127;177 | functions/src/consoleMetadata.ts; functions/src/consoleMetadata.test.ts; functions/src/shipDamage.ts; src/data/vessels/templates.ts; src/data/vessels/aegis.ts; src/data/aegisConsoles.ts; src/data/roleProcedures.ts; src/data/vesselTemplates.test.ts; src/data/aegisConsoles.test.ts; private routed ship-system derivative | The server-only CONSOLE_METADATA catalog derives one typed record for every configured damage-deck console, including its card identity, phase, maintenance step, charge requirement, printed damage/upgrade/effect rules, and implemented or deferred authoritative resolver. Client vessel and AEGIS definitions carry only an opaque shipId:systemId lookup key, preserving the existing no-card client-catalog boundary. Implemented resolver links cover damage.draw, maintenance.storage, maintenance.reactor, maintenance.bays, maintenance.production (Dione and expansion Capybara production), and jump.resolve. Deferred action links are explicit: aegis:construction-bay -> 178; aegis:fighter-bay-alpha, aegis:fighter-bay-bravo, aegis:command-and-control, aegis:missile-launchers, and aegis:point-defence-lasers -> 182; dione:vip-lounge -> 190/191; dione:fighter-bay -> 192; icebreaker:hydroponics -> 198; icebreaker:water-reclamation -> 199; icebreaker:mining-drone-control -> 200; icebreaker:ram-scoop -> 202; shepherd:water-reclamation -> 208; shepherd:advanced-hydroponics and shepherd:advanced-hydroponics-ii -> 209; quellon:hydroponics -> 220; quellon:water-production and quellon:water-production-ii -> 221; refinery-124:hydroponics -> 228; refinery-124:water-reclamation -> 229; refinery-124:fuel-refinery and refinery-124:fuel-refinery-ii -> 230; refinery-124:fighter-bay -> 231. Identity-only Gorgoneion, base Capybara, Warrior, Vulcan, and Voyage 33-0 console sets remain explicitly unregistered for their later vessel prompts. P165 is partial because the metadata links are not a substitute for those absent action resolvers. |
...
| E-AUDIT-272 | hard_prompt | 272 -> 161;162 | src/data/vessels/chepu.ts; src/data/shuttles.ts; functions/src/shuttlecraft.ts; functions/src/craftOwnership.ts; src/data/shuttles.test.ts; functions/src/shuttlecraft.test.ts; functions/src/craftOwnership.test.ts; src/routes/ShuttleConsole.tsx; src/routes/ShuttleConsole.test.tsx; authorized routed Chepu derivative | Chepu registration preserves its PDF Colonel owner, Refinery 124 dock, security-team cargo, boarding support, and fuelled Boarding Action relocation; the routed shuttle derivative leaves mission participation and numeric bonus unresolved. |
...
| E-416A-SOURCE-PARTIAL | evidence | 416a | src/data/missionCards.ts; src/data/missionCards.test.ts | The typed catalog encodes Ice Asteroids B as a poor six-card mission with two opportunities: exploration 17 for 6 water, and mining 24/30 for 8 water plus a critical +1 material bonus. The routed source does not establish a B-specific trait-bonus value or score-failure consequence, so both fields remain explicit unresolved values and must fail closed in future resolution. The focused catalog suite passes 4 tests plus typecheck; no dealing, resolution, player-facing flow, or release is claimed. |
| E-416B-SOURCE-PARTIAL | evidence | 416b | src/data/missionCards.ts; src/data/missionCards.test.ts | The typed catalog encodes Rare Element Moon C as a poor six-card mission with two opportunities: mining/exploration 20 for 2 minerals, and science 25 for the Endeavour to cross out 1 research box of the player's choice. The routed source inventory does not preserve a primary mission-card PDF, and its derivative does not establish a C-specific trait-bonus value or score-failure consequence. Those fields remain explicit unresolved gaps rather than inferred defaults, so Prompt 416b is partial; mission dealing and resolution remain deferred. |
...
| E-314-POD-PRODUCER | hard_prompt | 314 -> 140d | docs/implementation-prompts.json (Prompts 314 and 140d); functions/src/index.ts; src/types/game.ts | Removing destroyed vessels while keeping escape pods addressable requires the durable catastrophe and pod-capacity record from Prompt 140d. Current destruction paths record ship damage and a ship-destroyed event but do not create escape-pod state; hiding a DRADIS contact alone cannot satisfy the acceptance. This records implementation readiness only, not a commit, merge, push, or deployment gate. |
| E-314-NAVIGATION | evidence / authority / related/consumes | 314 -> 285;140d | functions/src/index.ts; functions/src/jumpCallable.test.ts; functions/src/shipDestruction.ts; src/data/fleetFormation.ts; src/data/fleetFormation.test.ts; src/components/ShipPlot.tsx; src/components/ShipPlot.test.tsx; src/App.tsx; src/routes/GmConsole.tsx; focused Functions/unit tests; /tmp/p314-render-evidence.txt; docs/implementation-prompts.json (Prompts 285, 314, and 140d) | Full-ship destruction state now denies authoritative moveShipToLocation and jumpShip mutations before fuel, charge, coordinate, or navigation-log writes, and fleetViewFrom excludes destroyed full ships while preserving active surviving contacts. Existing P140d catastrophe writes remain authoritative for ship-destroyed pod capacity, survivors, and retained shuttle records, so those paths stay addressable. Base and optional small-vessel destruction producers/capacity remain unresolved and keep P314 partial; no small-vessel behavior is inferred. |
