...
| E-284-GROUP-PRODUCER | hard_prompt | 284 -> 286 | docs/implementation-prompts.json (Prompts 284 and 286); functions/src/index.ts; src/types/game.ts | Fleet-group entitlement requires authoritative fleet-group identity before unknown-system facts can be projected to the correct group. The current code has no fleet-group state; a client-only visibility filter cannot satisfy the acceptance. This records implementation readiness only, not a commit, merge, push, or deployment gate. |
