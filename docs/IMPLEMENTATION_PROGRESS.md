...
**130 / 747 prompts complete (17.40%)**

Status breakdown: **130 done · 6 partial · 611 missing**.
...
| 111 | done | non-feature | — | Authoritative resource ledgers now parse ore, fuel, food, water, materials, security teams, and Capybara-only Scrap as nonnegative safe integers. GM-only resource callables apply ordered +/-1 changes transactionally and re-read the latest count after a retry; malformed present values become zero while absent legacy fields retain printed starting stock. Jump and maintenance consume the same parsed ledgers, and Firestore denies direct session writes. Damage cards remain a server-owned immutable deck projection: only known damaged system IDs persist and legacy client-supplied deck order is ignored. Focused tests cover malformed values, client projection, GM authority, ordered batches, retry/re-read behavior, expansion-only Scrap, exact printed stock, and retry-safe card draws. Proof-only; no player release. |
