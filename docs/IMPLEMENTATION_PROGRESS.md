...
Status breakdown: **283 done · 25 partial · 443 missing**.
...
| 106c | done | non-feature | 0.3.48 | The FleetTicker lifecycle now uses one server-owned stream for automatic, Admiral, and Press transmissions. Deterministic session/message/revision identities, precedence, current/queued/draining state, dismissals, replay cursors, and server deadlines serialize concurrent send/replace/dismiss plus Red Alert activation/stand-down, including the terminal/debrief producer. Reconnect, replay, and late join converge on server order/status; each viewport scrolls visible tails at constant speed without client frame or visual-pass writes. Red Alert stand-down expires after a shared 60-second server lifetime. Authenticated authority, idempotent CAS, privacy-safe projections/audit, schema bounds, and direct-write denial are covered before P652a/P652b. |
...
| 114 | partial | non-feature | 0.3.52 | The implemented full-vessel maintenance path now reads a registered order: AEGIS presents and authorizes Shuttle Bay Zeta at step 6 then Omega at step 7, while the other six full ships execute their printed 1–6 lane before explicit cycle closure. Focused server callable/state tests cover out-of-order bay and end actions, duplicate and retry-safe lifecycle, revision authority, and rendered AEGIS separation. Five supplemental identities are still metadata-only and therefore the each-vessel acceptance remains partial: Gorgoneion, base Capybara, Warrior, and Vulcan need shared small-ship runtime (P234) plus variant work (P235/P241/P242/P246), and Voyage 33-0 needs admission and maintenance (P249/P250); the population-loss exception belongs to P121. No unimplemented gameplay is implied. |
...
| 165 | partial | non-feature | — | Partial metadata foundation: Functions-only `CONSOLE_METADATA` now gives each configured damage-deck console a server-side card, phase, charge step, charge/damage/upgrade/effect contract, and an explicit implemented or deferred resolver link. Client vessel and AEGIS definitions carry only opaque `shipId:systemId` lookup keys, so the existing no-card client catalog remains intact. Existing authoritative links cover damage draws, Storage, Reactor charging, shuttle-bay fuelling, Dione production, expansion Capybara Advanced Hydroponics, Water Production, and Scrap Refinery, and Jump Drive resolution. Deferred action links are recorded exactly for AEGIS Construction Bay (178); AEGIS combat consoles and Fighter Bays (182); Dione VIP Lounge and Fighter Bay (190-192); Icebreaker Hydroponics, Water Reclamation, Mining Drone Control, and Ram Scoop (198-202); Shepherd production (208-209); Quellon production (220-221); Refinery 124 production and Fighter Bay (228-231). Identity-only small ships and Voyage 33-0 have no registered console definitions yet and are tracked against their later vessel prompts. Focused Functions metadata, client key, AEGIS, and vessel-catalog tests pass; no gameplay release is claimed until the deferred action resolvers land. |
