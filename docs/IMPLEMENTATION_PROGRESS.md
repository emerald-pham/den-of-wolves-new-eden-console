...
| 623 | done | non-feature | — | Verified the existing HashRouter and session-route guards preserve static-host hash deep links. Focused App/PWA coverage proves anonymous protected routes return to the landing route, authenticated route restoration stays inside the allowlisted session surface, and invalid stored routes fall back safely. A real static HTTP host served dist through Chrome and WebKit at the authenticated AEGIS route, including reload, anonymous protected and unknown hashes, and an authenticated disallowed GM hash; no production change or gameplay write was needed. |
