...
**202 / 749 prompts complete (26.97%)**

Status breakdown: **202 done · 19 partial · 528 missing**.
...
| 282a | missing | feature | — | Owner-requested GM-console star-map selection with explicit pre-start locking. Current P282 supplies authoritative A/B/C configuration and the GM overlay, but confirmRoster resubmits the existing chart and has no chooser. Further source inspection shows configurationLocked is only set by startGame, so a pre-start chart-only lock is also missing. Implement both the usable selector and server-enforced chart lock without prematurely locking roster work or reverting shipped map behavior. |
