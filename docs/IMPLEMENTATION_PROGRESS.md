...
implementation status. P660 and P661 are now complete as tooling-only
...
| 014 | done | non-feature | — | Shared session-authority cursors preserve visibly stale cached rendering while rejecting stale/late callable, listener, queued, and secondary-projection results from authorizing mutations or overwriting newer authority. Focused rebased-candidate coverage: 253 tests passed across session, Firestore, airspace, secondary mutation, App, and header surfaces. |
