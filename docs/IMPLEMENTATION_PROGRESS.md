...
**267 / 751 prompts complete (35.55%)**

Status breakdown: **267 done · 21 partial · 463 missing**.
...
| 533 | done | non-feature | — | The facilitator can record an explicit private Religious Zealotry outcome at the current debated crisis revision using any non-empty unique subset of leave, pressure, investigate, and arrest, a bounded custom response, or both. The response links only the authoritative social-deduction census revision when present (or records its absence), preserves request history and replay authority, and keeps rationale, publication, binding-law handling, and automatic loyalty or suspicion effects separate. |
