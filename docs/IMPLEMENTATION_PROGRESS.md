...
| 518 | partial | non-feature | — | Partial: the production Commissar consent and purge path now applies the printed survivor-track loss followed by unrest reduction with exact current-captain UID consent, server-only state, sanitized current-role projections, active-game and turn guards, revision/CAS, replay, and once-per-ship-per-turn checks. Focused callable and production-panel tests plus 81 emulator-backed Firestore-rule cases cover handover revocation, raw-state denial, and private projections; narrow phone, desktop, and landscape reduced-motion render evidence is recorded. Independent authority review remains. |
