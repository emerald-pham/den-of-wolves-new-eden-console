...
- **Prompt 012 — [PRESERVE] Define command idempotency.** Acceptance: retrying a command returns its original result without duplicating state, cost, randomness, or audit events.
  - **M1 command matrix (audited 2026-09-10):** A command is a production callable that accepts a caller-provided `requestId`; a receipt binds action, session (or the pre-session creation boundary), request ID, actor, applicable GM instance, expected revision, and every mutation field before it can replay. Receipts and fingerprints are server-only; member events are redacted audit projections.

    | M1 boundary | Receipt / replay boundary | Retry and conflict proof |
    | --- | --- | --- |
    | Create session | `sessionCreationRequests/{uid}_{requestId}` | Canonical creation payload is checked before join-code/session random draws; an exact retry is read-only and a changed payload is denied. |
    | Join / resume | No caller request ID; atomic membership/presence convergence | This is not a receipt command. The callable resolves the same authenticated membership without publishing session enumeration; a future request-ID API must not be inferred here. |
    | Complete setup and facilitator responsibilities | Domain receipt plus server-only `commandReceipts/{requestId}` marker | Existing actor/instance/revision/payload receipts authorize before replay and occupy the shared action namespace; public events contain only audit fields. |
    | Casting preference, role assignment, and role release | `commandReceipts/{requestId}` | Exact retries replay one result; changed payloads and cross-action IDs fail without a write. Role release authorizes and checks the receipt before reading loyalty secrets. |
    | Loyalty assignment and Android proof | `loyaltyAssignmentRequests` plus `commandReceipts/{requestId}` | Prompt 055's private loyalty binding remains authoritative and participates in shared action collisions; Android proof validates the current private holder before replay. |
    | Seat claim / release | Domain receipt plus server-only `commandReceipts/{requestId}` marker | Existing actor, action, seat, revision, and payload binding preserves one winner and stale/foreign denial across the shared namespace; fingerprints remain private. |
    | Press availability | `commandReceipts/{requestId}` | Actor, GM instance, revision, and `pressEnabled` are bound; conflicting/foreign reuse cannot disclose the prior result or write a second audit event. |
    | Start and Turn 1 | Domain receipt plus server-only `commandReceipts/{requestId}` marker | Existing actor/instance/revision receipt returns its committed start result without repeating Turn 1, setup secrets, or audit writes, and cannot be reused by another command. |
    | Later active-phase maintenance, rollback, and expiry | Their established domain receipts | Audited as request-bearing production mutators outside the M1 create → join → cast → start route; this prompt preserves their existing replay contracts rather than inferring a later-domain migration. |

  The M1/first-100 audit bounds the shared receipt namespace to the M1 boundaries above. Disabled partial setup mutators are not commands and the later attack, evacuation, candidate, and disconnect domains remain outside this contract. `commandIdempotency.test.ts`, focused callable tests, and the rules denial matrix cover exact replay, payload/revision/action/actor collisions, stale authority, concurrent transaction serialization, no duplicate audit writes or random draw on retry, redacted events, and direct client denial of `commandReceipts`.
